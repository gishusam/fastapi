...
from .. import models,schemas,database,utils,auth2
...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(
    auth2.get_current_user),limit: int = 10, skip: int = 0,search: Optional[str]= ""):
...
def get_post(id: int,db: Session = Depends(get_db),current_user: int = Depends(
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail= f"This post {id} is not found")
    return post
...
def delete_posts(id: int,db: Session = Depends(get_db),current_user: int = Depends(
    ...
    if post == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"The post with the index {id} doesn't exist")
    
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Not authourized to perform the operation")
...
def update_post(id: int, updated_post:schemas.PostCreate,db: Session = Depends(get_db),current_user: int = Depends(
    ...
    if post == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            details =f"The post with the index {id} doesn't exist")
